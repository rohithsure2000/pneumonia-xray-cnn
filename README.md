...
_Table above: the original team's reported results (see [Project origin](#project-origin)).
This repo's own reproduction of the Improved CNN, after fixing a validation-set
issue described below, reached **81.89% accuracy / 86.26% F1** -- see
[`docs/results.md`](docs/results.md#reproduced-results-this-repo) for the full
breakdown and why an exact match to the historical number isn't expected._
...
- No Grad-CAM or other interpretability output yet, which matters for a
  medical-imaging use case where a clinician would want to see _why_ a
